normalizar_chave_bairro <- function(x) {
  ...
  x <- gsub("\\bres\\b|\\bresid\\b", "residencial", x)
  x <- gsub("\\bvl\\b|\\bvil\\b", "vila", x)
  x <- gsub("\\bprc\\b", "praca", x)
  x <- gsub("\\bestr\\b", "estrada", x)
  x <- gsub("\\bfaz\\b", "fazenda", x)
  # Corrige variacoes ortograficas comuns (y↔i, ph↔f, etc)
  x <- gsub("goytacaz", "goitacaz", x)
  x <- gsub("jockei|joquei|jokey|joqei", "jockey", x)
  x <- gsub("alphavil", "alphaville", x)
  x <- gsub("travessao|travess.o|traves.o|travess o", "travessao", x)
  x <- gsub("farol de s o|farol de sao|farol sao|farol s o", "farol de sao tome", x)
  x <- gsub("custodopol|costodopol", "custodopolis", x)
  x <- gsub("\\bgoitaca\\b", "goitacazes", x)
  x <- gsub("\\bguarus\\b|\\bguaruz\\b", "guarus", x)
  x <- gsub("\\bpenha\\b", "penha", x)
  x <- gsub("\\bpelinca\\b|\\bpelinca\\b", "pelinca", x)
  x <- gsub("\\btocos\\b|\\btoco\\b", "tocos", x)
  x <- gsub("\\btapera\\b", "tapera", x)
  x <- gsub("\\bururai\\b|\\buruhai\\b", "ururai", x)
  x <- gsub("\\bdonana\\b", "donana", x)
  x <- gsub("\\bgarga[uú]", "gargau", x)
  x <- gsub("\\bc[oô]d[ií]n", "codim", x)
  x <- gsub("\\b[íi]nd[ií]gena\\b", "indigena", x)
  x <- gsub("m[uú]ssurep[eê]", "mussurepe", x)
  # Remove caracteres nao alfanumericos e normaliza espacos
  x <- gsub("[^a-z0-9]+", " ", x)
  gsub("\\s+", " ", trimws(x))
}

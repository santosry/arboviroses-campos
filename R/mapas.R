normalizar_chave_bairro <- function(x) {
  ...
  x <- gsub("travessao|travess[^a-z]*o|traves[^a-z]*o", "travessao", x)
  x <- gsub("farol de s[^a-z]*o|farol de sao|farol sao|farol s[^a-z]*o", "farol de sao tome", x)
  x <- gsub("custodopol|costodopol", "custodopolis", x)
  x <- gsub("gar[gc]au", "gargau", x)
  x <- gsub("codim|c[o0]dim", "codim", x)
  x <- gsub("mussurep", "mussurepe", x)
